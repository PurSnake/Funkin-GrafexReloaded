...
class StatisticMonitor extends openfl.display.Sprite
{
	/**
	...
	**/
	...
	public static var showMem:Bool = false; // TODO: Rename
	...
	public function new(x:Float = 10, y:Float = 10, color:Int = 0x000000)
	{
		...
		fpsText.defaultTextFormat = new TextFormat("_sans", 15);
	...
	private #if !flash override #end function __enterFrame(d:Float):Void
	{
		...
		if (currentFPS != fpsCount)
		{
			...
			if(showFPS && showMem) {
				...
				if (FlxG.state.subState != null) fpsText.text += '\nSubstate: ${Type.getClassName(Type.getClass(FlxG.state.subState))}';
			...
			for (outline in outlines) outline.text = fpsText.text;
		}
		fpsCount = currentFPS;
	}
